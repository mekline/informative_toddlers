...
```{r}
session_data_all = read_csv("C:/Users/babyg/Documents/data/processed_data/step2_TOCODE/v1jan2024-step2_all-responses-identifiable.csv", show_col_types = FALSE)
```

```{r loading-in-files}
#setwd("C:/Users/babyg/Documents/GitHub/informative_toddlers/data/v1jan2024_preregistered_dataset")
raw_df <- read_csv("C:/Users/babyg/Documents/study-v1jan2024_coding_all_orders_3_11_data.csv", show_col_types = FALSE)
...
# LW: I do not understand the purpose of this code chunk as it does not do anything except add a new column
```
...
trial_data_h1 <- trial_data_toanalyze %>%
  ...
  select(session_id, child_id, stimulus_condition, trial_num, stimulus_set, response_included_informative, response_included_uninformative)
```
...
trial_data_h2 <- trial_data_toanalyze %>%
  ...
  select(session_id, child_id, trial_num, stimulus_condition, stimulus_set, response_exact_utterance, response_included_subject, response_included_object)
trial_data_h2
```
...
test_trials_to_analyze <- trial_data_toanalyze %>% 
  filter(trial_type == "test_trial",
         response_attempted_target == TRUE,
         !is.na(response_exact_utterance))
```

Trying to make graph for H1
```{r}
# number of children included in this graph
length(unique(relevant_test_trials$child_id))
...
inf_vs_cg_data <- data.frame(
  Category = c("Informative", "Common Ground"),
...
inf_vs_cg_data$Category <- factor(inf_vs_cg_data$Category, levels = c("Informative", "Common Ground"))
...
raw_mean_age = mean_age = mean(raw_df$child__age_rounded)/31
raw_gender_counts = table(raw_df$child__gender)
...
# informative_cg_percent <- sum( relevant_test_trials$response_included_informative == TRUE & relevant_test_trials$response_included_uninformative == TRUE ) / nrow(relevant_test_trials) * 100
# 
# # how often did they say something only informative
# informative_only_percent <- sum(
#   relevant_test_trials$response_attempted_target == TRUE & relevant_test_trials$response_included_informative == TRUE & relevant_test_trials$response_included_uninformative == FALSE ) / nrow(relevant_test_trials)
# 
# # how often did they say something only cg
# cg_only_percent <- sum(
#   relevant_test_trials$response_attempted_target == TRUE & relevant_test_trials$response_included_informative == FALSE & relevant_test_trials$response_included_uninformative == TRUE ) / nrow(relevant_test_trials) * 100
```
...
inf_and_cg_vs_inf_vs_cg_data <- data.frame(
  ...
  percentages = c(informative_cg_percent, informative_only_percent, cg_only_percent))
...
# making graph
ggplot(inf_and_cg_vs_inf_vs_cg_data, aes(x = type, y = percentages, fill = type)) +
  geom_bar(stat = "identity") +
  labs(y = " ", x = " ", fill = "Information Type") + 
  theme(panel.background = element_blank())
```
