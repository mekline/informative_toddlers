...
# temp step to add in the awareness coding
UROP_df <-  read_csv(paste0(data_location, '/UROP_', study_version,'_step3_all-speakers_CODED.csv'), show_col_types = FALSE)
...
# doing a merge to add awareness coding to the file - might change this flow later idk
trial_data_toanalyze <- trial_data_toanalyze %>%
  left_join(UROP_trial_data_all %>% select(session_id, frame_id, aware),
            by = c("session_id", "frame_id"))


# adding age and gender to the trial dataset from the session data set
...
trial_data_toanalyze <- trial_data_toanalyze %>%
  select(session_id, family_id, child_id, age_in_days, gender, coder, visual_shown,
         ...
         response_exact_utterance, parent_transcription, response_attempted_target, response_included_informative, response_included_cg, aware,
         include_trial, exclusion_reason, trial_notes)

# replace all NAs in the "included..." columns with just "n"
trial_data_toanalyze <- trial_data_toanalyze %>%
  mutate_at(
    vars(contains("included")), ~replace_na(na_if(., NA), "n")
  )
...
# do some data cleaning to recode e.g. yes/no into values that R can coerce later. But leave weird business so we catch oddly coded things!, don't force to T/F yet
...
trial_data_h1 <- trial_data_toanalyze %>%
  ...
  select(session_id, child_id, trial_id, age_in_months, gender, session_order, test_trial_type, trial_num, animacy, response_exact_utterance, response_included_informative,response_included_cg, aware, utterance_type)
...
```{r utterance type x trial type, utterance type x trial x age}

# utterance type x trial
...
```{r utterance type x animacy, utterance type x animacy x age}

# animacy 
ggplot(trial_data_h1, aes(x = animacy, fill = animacy)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Animacy", x = "Animacy", y = "Number of Utterances", fill = "Animacy\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
ggplot(trial_data_h1, aes(x = animacy, fill = utterance_type)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Utterance Type and Animacy", x = "Animacy", y = "Number of Utterances", fill = "Utterance Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 50, by = 5),
...
# animacy x age
ggplot(trial_data_h1, aes(x = age_in_months, fill = animacy)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Animacy and Age", x = "Animacy", y = "Number of Utterances", fill = "Animacy\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 50, by = 5),
...
# utterance type x animacy x age
ggplot(trial_data_h1, aes(x = age_in_months, fill = utterance_type)) +
  geom_bar(position = "dodge") +
  facet_wrap(~ animacy)+
  labs(title = "Utterances by Utterance Type, Animacy, and Age", x = "Age in Months", y = "Number of Utterances", fill = "Utterance Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 50, by = 5),
