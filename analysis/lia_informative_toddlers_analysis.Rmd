...
* There are `r length(unique(session_data_all$session_id))` unique responses are in the session-level data.
...
Of these `r length(unique(session_data_toreport$session_id))` sessions, `r length(unique(trial_data_all$session_id))` have had their responses coded so far. (`r nrow(filter(is.na(to_code)))` according to the corresponding columns in the session-level data; these numbers should match!)
