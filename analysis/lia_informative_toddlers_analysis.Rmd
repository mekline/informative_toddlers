...
trial_data_toanalyze <- trial_data_toanalyze %>%
  select(session_id, family_id, child_id, age_in_days, gender, coder, visual_shown,
         ...
         response_included_informative, response_included_uninformative, aware,
...
kable(head(utterance_summary_table, col.names = c("target", "condition", "informative", "uninformative", "utterance", "attempted_target", "include")))
```
...
kable(head(utterance_coding_table, col.names = c("target", "condition", "utterance", "subject", "object", "informative", "uninformative", "notes", "include")))
```
...
trial_data_h1 <- trial_data_toanalyze %>%
  ...
  filter(response_attempted_target == "TRUE") %>% 
  # include commonground, agent-ambiguous, and patient-ambiguous trials, but not warmups
  filter(stimulus_condition %in% c("commonground","agent_ambig","patient_ambig")) %>% 
  # trial id is good for trouble shooting
  mutate(trial_id = paste0(session_id, trial_num),
         age_in_months = round((age_in_days / 365) * 12),
         utterance_type = case_when(
           response_included_informative == TRUE & response_included_uninformative == FALSE ~ "Informative" ,
           response_included_informative == FALSE & response_included_uninformative == TRUE ~ "Common Ground",
           response_included_informative == TRUE & response_included_uninformative == TRUE ~ "Both")
         ) %>% 
  select(session_id, child_id, trial_id, age_in_months, gender, stimulus_condition, trial_num, stimulus_set, uninformative_entities, response_exact_utterance, response_included_subject, response_included_object, response_included_distractor, response_included_informative, response_included_uninformative, aware, utterance_type)
```
...
trial_data_h2 <- trial_data_toanalyze %>%
  ...
  select(session_id, child_id, trial_num, stimulus_condition, stimulus_set, uninformative_entities, response_exact_utterance, response_included_subject, response_included_object, age_in_days) %>% 
  mutate(trial_id = paste0(session_id, trial_num))
...
# needed to answer a specific laura question about the bear shoes scenes
bear_shoes_duck_ball <- trial_data_h1 %>% 
  filter(stimulus_condition == "commonground")
```
## Demographic Graphs
...
ggplot(all_kids_in_months, aes(x = age_in_months)) +
  ...
  labs(title = "V1 All Participants by Months", y = "Number of Children", x = "Age in Months") +
  geom_vline(aes(xintercept = mean(age_in_months)),
            color="red", linetype = "dotted", linewidth = 2)
# a density plot bc I can
ggplot(all_kids_in_months, aes(x = age_in_months)) +
  geom_density(fill = "red", color = "#e9ecef", alpha = 0.8) + 
  labs(title = "V1 All Participants by Months", y = "Number of Children", x = "Age in Months")
...
ggplot(session_kids_in_months, aes(x = age_in_months)) +
  geom_histogram(binwidth = 1, color = "black", fill = "white") +
  labs(title = "V1 All Participants by Months (Post-Session Level Exclusions)", y = "Number of Children", x = "Age in Months") +
  geom_vline(aes(xintercept = mean(age_in_months)),
            color="blue", linetype = "dotted", linewidth = 2)
# and a density plot
ggplot(session_kids_in_months, aes(x = age_in_months)) +
...
ggplot(trial_data_h1, aes(x = age_in_months)) +
  geom_histogram(binwidth = 1, color = "black", fill = "white") +
  labs(title = "V1 All Participants by Months (That Made it into the Model)", y = "Number of Children", x = "Age in Months") +
  geom_vline(aes(xintercept = mean(age_in_months)),
            color="purple", linetype = "dotted", linewidth = 2)
# and a density plot
ggplot(trial_data_h1, aes(x = age_in_months)) +
  geom_density(fill = "purple", color = "#e9ecef", alpha = 0.8) + 
  labs(title = "V1 All Participants by Months (That Made it into the Model)", y = "Number of Children", x = "Age in Months")
```
...
ggplot(trial_data_h1, aes(x = utterance_type, fill = utterance_type)) +
  ...
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
    limits = c(0, 100)) + 
  scale_fill_manual(values = c("violetred3", "royalblue3", "mediumpurple3")) +
  theme(axis.text.x = element_blank(),
      axis.ticks.x = element_blank(),
      plot.title = element_text(hjust = 0.5)) 

# responses specifically for the "commonground" scenes
ggplot(bear_shoes_duck_ball, aes(x = utterance_type, fill = utterance_type)) +
  geom_bar() +
  labs(title = "Relevant Utterances by Type in Bear-Shoes and Duck-Ball Scenes", x = "Utterance Type", y = "Number of Utterances", fill = "Utterance Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
```{r utterance type x age, utterance type x gender}

# utterance type x age line graph
...
# utterance type x gender bar graph 
...
```{r animacy, utterance type x animacy, animacy x age, animacy x trial type}

# adding a special animacy column right here
h1_animacy = trial_data_h1 %>% 
  mutate(
    animacy = case_when(
      # when kids mention both animate and inanimate features
      
      # kids mention subject OR the distractor when the trial is agent ambig
      (response_included_subject == TRUE | (response_included_distractor == TRUE & stimulus_condition == "agent_ambig")) & 
        # kids mention the object OR the distractor in patient ambig trials 
        (response_included_object == TRUE | (response_included_distractor == TRUE & stimulus_condition == "patient_ambig")) ~ "both",
       
      # kids mention subject OR the distractor when the trial is agent ambig
      response_included_subject == TRUE | (response_included_distractor == TRUE & stimulus_condition == "agent_ambig") ~ "animate",
      
      # kids mention object OR the distractor in patient ambig trials
      response_included_object == TRUE | (response_included_distractor == TRUE & stimulus_condition == "patient_ambig") ~ "inanimate"
    )
  )

# animacy
ggplot(h1_animacy, aes(x = animacy, fill = animacy)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Animacy", x = "Animacy", y = "Number of Utterances") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 110, by = 10),
    limits = c(0, 110)) + 
  scale_fill_manual(values = c("darkgoldenrod","darkred", "darkorange")) +
  theme(axis.ticks.x = element_blank(),
      plot.title = element_text(hjust = 0.5))

# utterance type x animacy
ggplot(h1_animacy, aes(x = animacy, fill = utterance_type)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Type and Animacy", x = "Animacy", y = "Number of Utterances", fill = "Utterance Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
# animacy x age
ggplot(h1_animacy, aes(x = age_in_months, fill = animacy)) +
  geom_bar(position = "dodge") +
  facet_wrap(~ animacy)+
  labs(title = "Utterances by Age and Animacy", x = "Age in Months", y = "Number of Utterances", fill = "Animacy\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 50, by = 5),
...
# animacy x trial type
ggplot(h1_animacy, aes(x = animacy, fill = stimulus_condition)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Animacy and Trial Type", x = "Animacy", y = "Number of Utterances", fill = "Trial Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
```{r awareness, utterance type x awareness}
# general awareness
ggplot(trial_data_h1, aes(x = aware, fill = aware)) +
  geom_bar() +
  labs(title = "Utterances by Child Awareness", x = "Awareness", y = "Number of Utterances") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
# utterance type x awareness
ggplot(trial_data_h1, aes(x = aware, fill = utterance_type)) +
  geom_bar(position = "dodge") +
  labs(title = "Utterances by Child Awareness and Type", x = "Child Awareness", y = "Number of Utterances", fill = "Utterance Type\n") +
  theme_bw() + 
  theme(panel.border = element_blank()) +
  scale_y_continuous(
    breaks = seq(0, 100, by = 10),
...
### Graph for H2a - Nothing came out of this hypothesis so we didn't do graphs for H2b or H2c

```{r}
# all of the distinct kids that apply to this hypothesis
# we do not use this for analysis/charts - we're using this to determine N, mean age, and age range!!!
 h2_kids<- trial_data_h2 %>% 
...
```{r h2a model plot}
h2a_ci <- confint(h2a_kids_model, method = "Wald") 
h2a_intercept_ci <- h2a_ci["(Intercept)", ]

plot_data_h2a <- tibble(val = "Selective kids",
                        estimate = log.odds.to.prob(fixef(h2a_kids_model)["(Intercept)"]),
                        lower.ci = log.odds.to.prob(h2a_intercept_ci[1]),
                        upper.ci = log.odds.to.prob(h2a_intercept_ci[2]))
...
### H2b
```{r h2b model plot}
h2b.kids.plot <- emmeans(h2b_kids_model, specs = "stimulus_condition", type = "response") %>%
  as_tibble()

ggplot(h2b.kids.plot %>% filter(stimulus_condition == "agent_ambig"), aes(x = stimulus_condition, y = prob)) +
  geom_errorbar(aes(ymin = asymp.LCL,
                    ymax = asymp.UCL), width = .05) +
  geom_point(size = 5, shape = 23, fill = "white") +
  ylim(0,1) +
  theme_bw() +
  labs(x = "", y = "Probability of being informative in ambiguous trials (among selective kids)") +
...
ggplot(h2b.kids.plot %>% filter(stimulus_condition == "patient_ambig"), aes(x = stimulus_condition, y = prob)) +
  geom_errorbar(aes(ymin = asymp.LCL,
                    ymax = asymp.UCL), width = .05) +
  geom_point(size = 5, shape = 23, fill = "white") +
  ylim(0,1) +
...
### H2c
```{r}
h2c_ci <- confint(h2c_kids_model, method = "Wald") 
h2c_intercept_ci <- h2c_ci["(Intercept)", ]

plot_data_h2c <- tibble(val = "Selective kids",
                        estimate = log.odds.to.prob(fixef(h2c_kids_model)["(Intercept)"]),
                        lower.ci = log.odds.to.prob(h2c_intercept_ci[1]),
                        upper.ci = log.odds.to.prob(h2c_intercept_ci[2]))
...
```{r}
speakers <- trial_data_h1 %>% 
  select(session_id, child_id, age_in_months, gender) %>% 
  distinct(session_id, .keep_all = TRUE)
 
write.csv(speakers,"C:/Users/babyg/MIT Dropbox/Lia Washington/MIT Work Files/ECCL/Informative Toddlers/Data/data/v1_jan2024/v1_jan2024_complete_dataset/processed_data/v1_jan2024_step2_TOCODE/speakers.csv")
```
