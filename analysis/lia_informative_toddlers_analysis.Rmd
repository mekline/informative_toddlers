...
### H2b
```{r h2b model plot}
h2b_ci <- confint(h2b_kids_model, method = "Wald")
h2b_intercept_ci <- h2b_ci["(Intercept)", ]

plot_data_h2b <- tibble(val = "Selective kids",
                        estimate = log.odds.to.prob(fixef(h2b_kids_model)["(Intercept)"]),
                        lower.ci = log.odds.to.prob(h2b_intercept_ci[1]),
                        upper.ci = log.odds.to.prob(h2b_intercept_ci[2]))
